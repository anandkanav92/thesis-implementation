...
class Imagenette(data.Dataset):
  ...
  def __init__(self,data_path,train=True,transform=None,target_transform=None):
    ...
    if self.train:
      ...
      if self.transform is not None:
        test_data = [self.transform(Image.open(x).convert('RGB')) for x in val_dir.glob('**/*') if (x.is_file() and x.parent.name is not "train" ) ]
        self.test_data = torch.stack(test_data)

      else:
        test_data = [Image.open(x).convert('RGB') for x in val_dir.glob('**/*') if (x.is_file() and x.parent.name is not "train" ) ]
        self.test_data = torch.stack(test_data)
